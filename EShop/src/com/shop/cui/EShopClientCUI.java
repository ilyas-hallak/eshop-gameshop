...
public class EShopClientCUI {
	...
	private boolean login(String enter) throws IOException {
		...
		try {
			...
			System.out
					.println("Die Eingabe ist fehlgeschlagen, wiederholen Sie den Vorgang.");
		}
		return false;
	}
	...
	private void processInput() throws IOException {
		...
		if (mitarbeiter == 1) {
			while(true) {
				...
				// Die Artikel Liste wird ueber die EShopV, nach Titel sortiert,
				// ausgegeben
				...
				// }
				/*
				 * Es wird ueber eine Eingabe nach einem Titel oder Nummer in der
				 * Artikel Liste gesucht
				 */
					...
					if (list == null) {
						System.out
								.print("Es wurde kein passender Artikel gefunden");
					} else {
			...
			do {
				...
				if (line.equals("w")) {
					...
					do {
						// hinzufügen von artikeln in den warenkorb
						List<Artikel> list = shop.getAllArtikel();
						giveOutArtikellist(list);
						System.out.print(" \n Geben Sie den Artieklnummer an:");
						String anr = readInput();
						...
						// Wenn ein Artikel mit der Anzahl 0 eingefügt wird,
